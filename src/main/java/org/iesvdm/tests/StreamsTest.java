...
class StreamsTest {
	...
	void test2() {
		...
		try {
			...
			lista.forEach(integer -> System.out.println(integer));
			cliHome.commitTransaction();
	...
	void test3() {
		...
		try {
			...
			//TODO STREAMS
			List<Float> lista = list.stream()
					.sorted(Comparator.comparing((Comercial comercial) -> comercial.getComisión()).reversed())
					.limit(1)
					.map(comercial -> comercial.getComisión())
					.toList();
	...
	void test4() {
		...
		try {
			...
			List<String> lista = list.stream()
					...
					.sorted(Comparator.comparing((Cliente c) -> c.getApellido1()).thenComparing((Cliente c) -> c.getApellido2()).thenComparing((Cliente c) -> c.getNombre()))
